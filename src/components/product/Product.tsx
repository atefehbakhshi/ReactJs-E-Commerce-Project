...
const Product: FC<ProductParams> = ({ img, title, price, id, quantity }) => {
  return (
    <Link to={`/products/${id}`}>
      <div className="cart-shadow cursor-pointer hover:scale-105 md:max-w-sm relative">
        {quantity === 0 && (
          <p className="text-red-400 font-light text-xs absolute top-6 left-0 rotate-[-45deg]">
            اتمام موجودی
          </p>
        )}
